...
const float gui::p2pX(const float perc, const sf::VideoMode& vm)
{
	...
	float perc				The percentage value.
	sf::VideoMode& vm		The current videomode of the window (resolution).
...
const unsigned gui::calcCharSize(const sf::VideoMode& vm, const unsigned modifier)
{
	...
	sf::VideoMode& vm		The current videomode of the window (resolution).
	unsigned modifier		Used to modify the character size in a more custom way.
...
/* ___________________________________________ DROP DOWN LIST _______________________________________________ */

gui::DropDownList::DropDownList(float x, float y, float width, float height, sf::Font& font, std::string list[], 
	unsigned nrOfElements, unsigned default_index) : font(font), showList(false), keytimeMax(1.f), keytime(0.f)
{
